...
A production-ready, pixel-close reproduction of the Figma "Assignment task" landing
...
## 5. Design system (from the brief)
...
## 6. Notes / assumptions worth knowing before you present this

- **No Figma Dev Mode access.** The shared link required a Figma sign-in to
  inspect exact pixel values, so this was rebuilt from your 4 screenshots +
  the written brief rather than measured layer-by-layer. Everything — colors,
  fonts, copy, spacing, section order — follows the brief exactly; only exact
  px-level spacing is an eyeballed match rather than an inspected one.
- **Images are placeholders, on purpose.** The hero "arena," the 6 discipline
  tiles, and the "What is BotLeague" illustration are hand-built SVG/CSS
  graphics (glows, circuit-line patterns, gradients) instead of hotlinked
  stock photos. That keeps the project dependency-free and copyright-safe.
  Drop real photos into `src/assets/images` and swap them into `Hero.tsx`,
  `SportsGrid.tsx`, and `AboutLeague.tsx` when you have real assets.
- **Sponsor logos** are generic monogram badges, not the real NIT/IIT crests
  — those are trademarked marks I can't source or reproduce. Swap in real
  logo files the same way once you have permission to use them.
- **"Become in Judge"** — the Figma text reads this way (likely meant to be
  "Become a Judge"). It's reproduced exactly as designed in
  `src/data/constants/index.ts`; change the `title` there if you want the fix.
- **React Router** is wired up with a single `/` route so the structure is
  ready to grow into a multi-page app later, even though today everything
  lives on one scrolling page.
- Nav links, hero buttons, and footer links scroll to real in-page sections
  (`#events`, `#programs`, `#ranks`, `#join`, etc.) rather than pointing
  nowhere.

## 7. Accessibility & performance touches included
